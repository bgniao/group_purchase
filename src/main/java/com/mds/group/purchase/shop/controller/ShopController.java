...
public class ShopController {
     ...
     * @return 返回结果
     */
    @PostMapping("/v1/upload/{type}/{mall}/{appmodelId}")
    public Result upload(
            @PathVariable String type,
            @PathVariable String mall,
            @PathVariable @NotBlank(message = "appmodelId不能为空") @Size(max = 27, min = 27, message = "商品标示错误") String appmodelId,
            @RequestParam("files") MultipartFile files
    ) throws IOException {
        String path = FileUploadUtil.uploadFile(files.getInputStream(), files.getOriginalFilename(), appmodelId);
